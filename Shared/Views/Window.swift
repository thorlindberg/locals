...
struct Window: View {
    ...
    @State var data: Storage.Format = Storage.Format(
        base: "", target: "", alerts: true, saved: "", filters: Storage.Format.Filters(unpinned: true, singleline: true, multiline: true, parenthesis: true, nummerical: true, symbols: true),
    ...
    var body: some View {
        NavigationView {
            Sidebar(toggle: $toggle, selection: $selection, status: $status, progress: $progress, data: $data,
                ...
                .frame(minWidth: 200)
            Editor(selection: $selection, status: $status, progress: $progress, data: $data, query: $query, entry: $entry)
        }
